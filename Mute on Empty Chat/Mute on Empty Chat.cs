...
Set first_check to this Code: */
	...
	if (count == 1) {
		msg = plugin.R("ATTENTION %k_n%! You entered blank spaces in chat, write some letters!");
		...
		plugin.Log("Logs/InsaneLimits_empty_chat.log", plugin.R("[" + fancy_date + "][" + fancy_time + "] %p_n% said: [" + player.LastChat + "]"));
	}
	if (count == 2) {
		...
		plugin.SendGlobalMessage(msg);
		plugin.PRoConChat("ADMIN MUTE > " + msg);
		plugin.ConsoleWrite("^b^1ADMIN MUTE >^0^n " + player.FullName + " tripped the Empty Chat filter! ");
		plugin.Log("Logs/InsaneLimits_empty_chat.log", plugin.R("[" + fancy_date + "][" + fancy_time + "] %p_n% said: [" + player.LastChat + "]"));
	}
	else if (count == 3) {
